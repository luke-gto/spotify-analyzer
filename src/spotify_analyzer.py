...
def last_played(songs_num):
    ...
    av_df = pd.DataFrame(
        {
            "average_danceability_%": av_dance,
            "average_energy_%": av_energy,
            "average_positivity_%": av_positivity,
            "average_tempo": av_tempo,
            "average_loudness_dB": av_loudness,
            "average_speechiness_%": av_speechiness,
            "average_instrument_%": av_instrument,
...
def top_artist(time_range, songs_num):
    ...
    results = sp.current_user_top_artists(
        limit=songs_num, offset=0, time_range=time_range
    )
    name_list = []
    genre_list = []
    followers_list = []
...
def export_ods(dataframe, av_df, working_directory, file_name):
    ...
    with pd.ExcelWriter(
        "spotify-" + file_name + "-" + str(get_time()) + ".xlsx",
        engine="xlsxwriter",
